...
describe UsersHelper do
  ...
  describe "if_authorized" do 
    it "yields if authorized" do
      @user.expects(:authorized?).with('a','r').yields('a', 'r').returns(true)
      @user.if_authorized?('a','r'){|action,resource| [action,resource,'hi'] }.should == ['a','r','hi']
    end
  ...
  describe "link_to_user" do
    ...
    it "should link to the given user" do
      expects(:user_path).returns('/users/1').at_least(1)
